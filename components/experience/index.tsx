...
export default function ExperienceSection({
  ...
  return (
    ...
    >
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(circle_at_18%_18%,color-mix(in_oklch,var(--chart-2)_16%,transparent),transparent_50%),radial-gradient(circle_at_88%_88%,color-mix(in_oklch,var(--chart-3)_16%,transparent),transparent_56%)]"
      />
      ...
      <div className="space-y-4">
        {experiences.map((item, index) => (
          <BlurFade key={`${item.company}-${item.period}`} inView delay={0.08 + index * 0.06}>
            <MagicCard
